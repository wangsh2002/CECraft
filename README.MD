...
### 2.2 简历管理与解析 (Resume Management)

简历数据的全生命周期管理，涵盖上传、解析、存储与版本控制。

* **文件上传与解析**：
    * 支持 PDF/Word/Markdown 等多种格式上传。
    * 后端集成解析引擎，提取文本内容并转换为系统内部的 Delta 数据结构，为后续的 AI 处理提供标准化的上下文。
* **结构化存储**：
