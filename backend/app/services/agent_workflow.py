...
2. **research_modify (调研并修改)**: 
    - 适用场景：用户希望**利用搜索到的外部信息或内部知识库（如STAR法则、优秀范例）来优化或修改**简历。
    - 关键词：根据JD修改、对标大厂、参考链接、STAR法则、专业化重写、参考优秀案例、根据搜索结果修改。
    - 示例："根据现在市场上 Java 高级的要求，帮我优化技能列表"、"用STAR法则把这段经历改得更专业"、"参考大厂简历风格修改"。

3. **modify (直接修改)**: 
    - 适用场景：用户指令明确，**不需要**额外参考资料，仅基于通用写作能力进行润色、精简、翻译或纠错。
...
- "search_query": (可选) 如果意图是 research_*, 请生成一个优化后的搜索关键词（例如："Java后端 面试题"、"STAR法则 简历范文"）。如果不需要搜索，返回空字符串。
"""
...
### ⚠️ 内容保持与格式约束 (重要)
1. **事实绝对保留**：**严禁**修改任何专有名词、公司名称、部门名称（如“开源团队”）、项目名称、奖项名称或数字。例如，不要将“腾讯犀牛鸟”改为“腾讯独角兽”或“腾讯熊猫鸟”，不要将“蚂蚁集团”改为“阿里云”。
2. **富文本格式保留**：请务必保留原有的富文本格式（如加粗、颜色、字体大小等），除非用户明确要求修改格式。
3. **大段空格保留**：文本中用于对齐或排版的大段连续空格，请**务必原样保留**，不要合并或删除。
4. **结构优化**：请保持简历的整体段落顺序。但是，**如果原内容的排版混乱，请务必进行结构化重构**。
    - 将手动输入的 "- " 转换为标准的列表格式。
    - **标点统一**：将中文语境下的英文标点（如 `,`）替换为中文标点（如 `，`）。对于并列的长短句，请使用分号 `；` 进行分隔，以提升层次感。
5. **英文规范**：**严禁**在英文单词内部插入错误的空格（如将 "ByteDance" 写成 "Byte Dance" 或 "字 节"）。
6. **润色力度**：当用户要求“润色”时，请**大胆优化**，不要仅限于纠错。
    - **专业化重写**：将口语化或简单的描述转化为简历专业用语。例如，将“代码贡献”改为“核心贡献者”或“主导开发”；将“若干知名项目”改为“深度参与多个业界主流开源项目”。
    - **结构化升级**：务必将文本中的伪列表（如使用 "—"、"-"、"*" 开头的行）转换为真正的 Quill 列表格式。
    - **排版修复**：修复混乱的标点（如将长句中的逗号改为分号以区分层次）、多余的空格（如“字 节”）。

### Delta 格式严格要求
修改后的数据 (modified_data) 必须是一个包含 "ops" 数组的对象。
**关键：请务必将原始属性转换为标准的 Quill/BlockKit 属性 (bold, fontSize, color, list)，不要保留原始业务字段。**
1. **列表处理**：当将文本转换为列表时，请使用 Quill 的 list 属性（例如 `{{ "attributes": {{ "list": "bullet" }}, "insert": "\\n" }}`），而不是手动输入 "• " 或 "- "。
2. **加粗处理**：**严禁**使用 Markdown 的 `**` 或 `__` 语法。必须使用 Quill 的 bold 属性。
    - ❌ 错误：`{{ "insert": "**熟悉HTML5**" }}`
    - ✅ 正确：`{{ "insert": "熟悉HTML5", "attributes": {{ "bold": true }} }}`
3. **混合样式**：如果一段文字中只有部分需要加粗，必须拆分为多个 ops 对象。
    - ❌ 错误：`{{ "insert": "**熟悉HTML5**, 了解CSS3" }}`
    - ✅ 正确：`[ {{ "insert": "熟悉HTML5", "attributes": {{ "bold": true }} }}, {{ "insert": ", 了解CSS3" }} ]`
